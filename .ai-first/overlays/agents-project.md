...
- Stage 0부터 Stage 8까지 완료됐다. 완료된 계약과 evidence는
  `docs/completed-milestones.md`와 `docs/milestones/`, 현재 상태와 다음 trigger는
  `docs/HANDOFF.md`와 `docs/status.md`가 소유한다.
- 현재 active milestone은 없다. 다음 trigger는 stable `v1.3.0` release 결정,
  consumer migration 승인 또는 새 capability profile 요구다.
