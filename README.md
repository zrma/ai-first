...
생성되는 운영 계약을 유지한다. 실제 출고·소비 적용 상태는 [handoff](docs/HANDOFF.md)를 따른다.

`1.7.2`는 고정된 검토 범위와 근거, 누적 finding 상태와 후발 지적의 기원 구분을
공통 generated 지침에 포함한다. 실제 리뷰 누락 감소 효과는 미측정이다.
